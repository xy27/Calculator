...
std::auto_ptr<Node> Parser::Expr()//解析表达式
{
...
std::auto_ptr<Node> Parser::Term()//解析项
{
	//Term :: = Factor{ ('*' | '/') Factor }
...
std::auto_ptr<Node> Parser::Factor()//解析因式
{
	...
	else if (token == TOKEN_INDETIFIER)	//Identifier
	{
		...
		else
		{
			...
			node = std::auto_ptr<Node>(new VariableNode(id, calc_.GetStorage()));//创建变量结点
		}
	...
	{
		scanner_.Accept();
		node = std::auto_ptr<Node>(new UMinusNode(Factor()));//这里有点奇怪，必须将构造函数的参数改为普通类型
	}
	else
	{
		status_ = STATUS_ERROR;
